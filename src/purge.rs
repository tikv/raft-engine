    ...
    P: PipeLog,
{
    ...
    fn rewrite_impl(
        ...
        let (file_version, file_id) = self.pipe_log.fetch_active_file(LogQueue::Rewrite);
        // Signs a checksum, so-called `[signature]`, into the LogBatch. The
        // `[signature]` is both generated by the given `[Version]` and
        // `[FileId]`. That is, the final checksum of each LogBatch consists of
