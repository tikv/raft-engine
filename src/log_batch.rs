...
impl LogBatch {
    ...
    ///
    /// Internally rewrites the checksum of each `LogBatch` by a signature,
    /// generated by the given `LogFileContext`. That is, the final checksum
    /// of each `LogBatch` consists of this `signature` and the original
    /// `checksum` of the contents.
    #[inline]
    pub(crate) fn prepare_write(&mut self, file_context: &LogFileContext) -> Result<()> {
        let new_signature = file_context.get_signature();
        match self.buf_state {
            BufState::Encoded(header_offset, entries_len) => {
                sign_checksum(&mut self.buf, new_signature)?;
                self.buf_state = BufState::Sealed(header_offset, entries_len, new_signature);
            }
            BufState::Sealed(header_offset, entries_len, old_signature) => {
                ...
                match (old_signature, new_signature) {
                    ...
                    (None, Some(new)) => {
                        sign_checksum(&mut self.buf, Some(new))?;
                    }
                    _ => {}
                }
                self.buf_state = BufState::Sealed(header_offset, entries_len, new_signature);
    ...
    pub fn approximate_size(&self) -> usize {
        if self.is_empty() {
            ...
            match self.buf_state {
                ...
                BufState::Sealed(header_offset, _, _) => self.buf.len() - header_offset,
                s => {
...
fn sign_checksum(buf: &mut Vec<u8>, signature: Option<u32>) -> Result<()> {
    if !buf.is_empty() {
