...
impl LogItemBatch {
    ...
    /// `signature` and the `checksum` of the contents.
    pub(crate) fn prepare_write(buf: &mut Vec<u8>, file_context: &LogFileContext) {
        if !buf.is_empty() {
            match file_context.version {
                Version::V1 => {
...
fn verify_checksum_with_context(buf: &[u8], file_context: &LogFileContext) -> Result<()> {
    ...
    let version = file_context.version;
    let expected = codec::decode_u32_le(&mut &buf[buf.len() - LOG_BATCH_CHECKSUM_LEN..])?;
    let actual = crc32(&buf[..buf.len() - LOG_BATCH_CHECKSUM_LEN]);
    ...
    if actual_with_context != expected {
        return Err(Error::Corruption(format!(
            ...
            expected, actual_with_context, version,
        )));
    }
    Ok(())
}
...
mod tests {
    ...
    fn test_log_batch_enc_dec() {
        fn decode_and_encode(
            ...
            batch.prepare_write(&file_context);
