...
impl LogItemBatch {
    ...
    pub(crate) fn prepare_write(buf: &mut Vec<u8>, format_version: Version, file_id: FileId) {
        if !buf.is_empty() {
                ...
                Version::V2 => {
                    ...
                    match codec::decode_u32_le(
                        ...
                        Ok(original_checksum) => {
                            let _ = (&mut buf[footer_checksum_offset - LOG_BATCH_CHECKSUM_LEN..])
    ...
    pub fn decode(
        ...
        verify_checksum_with_context(buf, verification.0, verification.1)?;
...
impl LogBatch {
    ...
    pub(crate) fn prepare_write(&mut self, format_version: Version, file_id: FileId) {
        debug_assert!(matches!(self.buf_state, BufState::Sealed(_, _)));
        if !self.is_empty() {
            LogItemBatch::prepare_write(&mut self.buf, format_version, file_id);
        }
    }
...
fn verify_checksum_with_context(
    buf: &[u8],
    version: Version,
...
mod tests {
    ...
    fn test_log_batch_enc_dec() {
        fn decode_and_encode(
            ...
            batch.prepare_write(version, batch_handle.id);
    ...
    fn test_verify_checksum() {
        ...
        });
        {
            ...
            .is_ok());
            assert!(
                verify_checksum_with_context(&data[..], Version::V2, signature.clone()).is_err()
            );
        }
        {
            ...
            data.encode_u32_le(checksum ^ signature.encode_as_u32())
                .unwrap();
            assert!(
                verify_checksum_with_context(&data[..], Version::V1, signature.clone()).is_err()
            );
            assert!(
                verify_checksum_with_context(&data[..], Version::V2, signature.clone()).is_ok()
            );
        }
    }
