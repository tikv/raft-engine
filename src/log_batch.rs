...
use crate::pipe_log::{FileBlockHandle, FileId, Signature};
...
impl LogItemBatch {
    ...
    pub fn decode(
        ...
        verification: (Version, Signature),
    ) -> Result<LogItemBatch> {
        // Validate the checksum of each LogItemBatch by the signature.
        verify_checksum(buf, verification.0, verification.1)?;
...
impl LogBatch {
    ...
    ///
    /// The `signature` is both generated by the given `[Version]` and
    /// `[FileId]`. That is, the final checksum of each `LogBatch`
    /// consists of this `signature` and the `checksum` of the contents.
    pub(crate) fn sign_checksum(&mut self, format_version: Version, file_id: FileId) {
        debug_assert!(matches!(self.buf_state, BufState::Sealed(_, _)));
        if !self.is_empty() {
            match format_version {
                Version::V1 => {
                    // default with `Version::V1`, nothing needs to be done.
                }
                Version::V2 => {
                        ...
                        BufState::Sealed(header_offset, len) => {
                            ...
                            match codec::decode_u32_le(
                                ...
                                Ok(original_checksum) => {
                                    let _ = (&mut self.buf[checksum_offset..checksum_offset + 4])
                                        .write_u32::<LittleEndian>(
                                        original_checksum ^ signature,
                                    );
                                }
                                _ => unreachable!(),
                            };
                            ...
                            match codec::decode_u32_le(
                                ...
                                Ok(original_checksum) => {
                                    let _ = (&mut self.buf
                                        [footer_checksum_offset - LOG_BATCH_CHECKSUM_LEN..])
    ...
    pub(crate) fn decode_entries_block(
        ...
        if handle.len > 0 {
            verify_checksum(&buf[0..handle.len], verification.0, verification.1)?;
            match compression {
...
fn verify_checksum(buf: &[u8], version: Version, mut signature: Signature) -> Result<()> {
    ...
    let expected = codec::decode_u32_le(&mut &buf[buf.len() - LOG_BATCH_CHECKSUM_LEN..])?;
    let actual = crc32(&buf[..buf.len() - LOG_BATCH_CHECKSUM_LEN]);
    let actual_with_signature = match version {
        Version::V1 => actual,
    ...
    */
    if actual_with_signature != expected {
        return Err(Error::Corruption(format!(
            ...
            expected, actual_with_signature, version,
        )));
    }
    Ok(())
}
...
mod tests {
    ...
    fn decode_entries_from_bytes<M: MessageExt>(
        ...
        for ei in entry_indexes {
            let block = LogBatch::decode_entries_block(
                ...
                ei.entries.unwrap(),
                ei.compression_type,
                (version, Signature::new(ei.entries.unwrap().id)),
            )
            .unwrap();
            entries.push(
                parse_from_bytes(
        ...
        for batch in batches.into_iter() {
            for compression_type in [CompressionType::Lz4, CompressionType::None] {
                ...
                let decoded_batch = LogItemBatch::decode(
                    ...
                    (
                        Version::default(),
                        Signature::new(FileId::dummy(LogQueue::Append)),
                    ),
                )
                .unwrap();
    ...
    fn test_log_batch_enc_dec() {
        fn decode_and_encode(
            ...
            // Verify the first part of LogBatch.
            // Attention: the parsed `offset` contained the LOG_BATCH_HEADER_LEN
            assert!(verify_checksum(
                &bytes_slice[..offset - LOG_BATCH_HEADER_LEN],
                version,
            ...
            let decoded_item_batch = LogItemBatch::decode(
                ...
                entries_handle,
                compression_type,
                (version, Signature::new(entries_handle.id)),
            )
            .unwrap();
    ...
    fn test_log_batch_merge() {
        ...
        let decoded_item_batch = LogItemBatch::decode(
            ...
            compression_type,
            (Version::default(), Signature::new(file_id)),
        )
        .unwrap();
    ...
    fn test_verify_checksum() {
        ...
        assert!(verify_checksum(
            &invalid_data[..],
            Version::V1,
            Signature::dummy(LogQueue::Append)
        )
        .is_err());
        let mut signature = Signature::new(FileId {
            seq: 11,
        ...
        });
        {
            ...
            assert!(
                verify_checksum(&data[..], Version::V1, Signature::dummy(LogQueue::Append)).is_ok()
            );
            assert!(
                verify_checksum(&data[..], Version::V2, Signature::dummy(LogQueue::Append)).is_ok()
            );
            assert!(verify_checksum(&data[..], Version::V2, signature.clone()).is_err());
        }
        {
            // Sign checksum and verify it with Version::V2
            let mut data: Vec<u8> = (0..128).map(|_| thread_rng().gen()).collect();
            let checksum = crc32(&data[..]);
            data.encode_u32_le(checksum ^ signature.encode_as_u32())
                .unwrap();
            assert!(verify_checksum(&data[..], Version::V1, signature.clone()).is_err());
            assert!(verify_checksum(&data[..], Version::V2, signature.clone()).is_ok());
        }
    }
